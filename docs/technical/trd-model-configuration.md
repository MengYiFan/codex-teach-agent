...
| `routing_priority` | 否 | 1-1000，值越小优先级越高；同一能力下优先级最小的启用配置为默认路由 |
...
8. 自定义 `base_url` 必须经过 SSRF 防护：只允许批准的 scheme/端口，解析和重定向后都检查目标 IP，默认禁止 loopback、链路本地、云元数据和未批准私网；本地模型使用显式环境 allowlist。
9. 外部调用统一经过受控出站代理或网络策略，不允许供应商响应把请求重定向到未批准域名。
...
同一能力下只考虑启用且满足数据区域/策略的配置，按 `routing_priority ASC, config_id ASC` 稳定排序；每次调用记录实际配置和 fallback 顺序，不能因并发或数据库返回顺序随机选模。
